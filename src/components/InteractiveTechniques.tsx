...
export const TechniquesDictionary: React.FC<TechniquesDictionaryProps> = ({ selectedAgeRange }) => {
  const [activeTab, setActiveTab] = useState<'generales' | 'relajacion' | 'concentracion'>('relajacion');
  ...
  const generalesParams = [
    ...
    },
    {
      ...
      example: 'Dividir una tarde de estudio en bloques concretos de actividades.'
    },
    {
    ...
    },
    {
      ...
      example: 'Preguntas guía: ¿He terminado la tarea? ¿Qué he hecho bien? ¿Qué puedo mejorar?'
    },
    {
  ...
  const relajacionParams = [
    ...
    },
    {
      ...
      steps: [
        ...
        'Mantener 4 segundos.',
        'Expulsar 4 segundos.',
        'Esperar 4 segundos antes de volver a empezar (Se visualiza imaginando un cuadrado).'
      ]
    },
    {
    ...
    },
    {
      ...
      example: 'Prestar atención durante unos minutos únicamente a cómo entra y sale el aire.'
    }
  ];

  const concentracionParams = [
    {
      title: 'Trabajo por intervalos cortos (Pomodoro)',
      desc: 'Permite mantener la atención durante tiempos breves.',
      steps: ['Elegir una tarea.', 'Trabajar 20–25 minutos.', 'Descansar 5 minutos.', 'Repetir ciclos.']
    },
    {
      title: 'Una tarea a la vez',
      desc: 'Evita la sobrecarga atencional.',
      steps: ['Elegir solo una actividad.', 'Retirar tareas secundarias.', 'Finalizar antes de comenzar otra.']
    },
    {
      title: 'Externalización de la atención (Checklist)',
      desc: 'Ayuda a mantener el foco usando apoyos externos.',
      steps: ['Elaborar una lista de pasos.', 'Ir marcando cada tarea completada.', 'Revisar el progreso.']
    },
    {
      title: 'Fragmentación de tareas',
      desc: 'Reduce sensación de dificultad.',
      steps: ['Dividir la tarea en pasos pequeños.', 'Completar un paso cada vez.', 'Reforzar avances parciales.'],
    ...
    },
    {
      ...
      steps: ['Apagar notificaciones.', 'Limitar pantallas innecesarias.', 'Trabajar en un lugar tranquilo.']
    },
    {
      title: 'Estimulación auditiva controlada',
      desc: 'Ayuda a bloquear distracciones sonoras usando ruidos blancos.',
      steps: ['Usar sonidos constantes (lluvia, ventilador, ruido blanco).', 'Mantener volumen bajo.', 'Comprobar si mejora la concentración.']
    },
    {
      title: 'Técnicas de inicio rápido',
      desc: 'Buscan reducir la evitación de tareas.',
      steps: ['Comprometerse a trabajar solo 2–5 minutos.', 'Empezar sin pensar en terminar.', 'Mantener el trabajo si aumenta la motivación.']
    },
    {
      title: 'Apoyos visuales',
      desc: 'Facilitan organización y comprensión.',
      steps: ['Utilizar esquemas, pictogramas o listas.', 'Colocar recordatorios visibles.', 'Señalar pasos o tiempos.']
    },
    {
      title: 'Alternar tarea con descanso',
      desc: 'Reduce la fatiga mental.',
      steps: ['Establecer periodos breves de trabajo.', 'Introducir descansos cortos.', 'Realizar actividades reguladoras durante la pausa (Ejemplo: estirarse, beber agua o caminar).']
    },
    {
      title: 'Respiración antes de iniciar una tarea',
      desc: 'Ayuda a preparar el cerebro para concentrarse.',
      steps: ['Detenerse un momento antes de empezar.', 'Realizar 3–5 respiraciones profundas.', 'Repetir mentalmente qué se va a hacer.', 'Iniciar la actividad.']
    }
  ];
  ...
  const getAdaptationText = () => {
    switch(selectedAgeRange) {
      case 'p1': return 'Adaptado para el 1º Ciclo (6-8 años): Fomentar el componente lúdico, las metáforas visuales (como el globo o la vela) y mantener los pasos muy cortos y guiados por el adulto.';
      case 'p2': return 'Adaptado para el 2º Ciclo (8-10 años): Reafirmar la autonomía apoyándose en registros visuales, comenzar a nombrar las emociones de forma directa e internalizar las autoinstrucciones.';
      case 'p3': return 'Adaptado para el 3º Ciclo (10-12 años): Mayor independencia en la ejecución de las técnicas de estudio, delegar el control del Pomodoro al alumno.';
      case 'b1':
      case 'b2': return 'Adaptado para Secundaria y Bachillerato: Aumentar la responsabilidad del estudiante, integrar mindfulness en el estudio intenso y usar "Checklists" digitales.';
      case 'a18': return 'Adaptado para Adultos (18-21 años): Orientado al autoconocimiento, prevención del "Burnout" académico, limitación proactiva del entorno digital y autorregulación activa.';
      default: return 'Recursos adaptativos en base a la edad.';
    }
  };
  ...
  return (
    <div className="mt-8 border-4 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-black">
      <div className="p-4 sm:p-6 border-b-4 border-black bg-amber-50">
        <h3 className="font-black text-2xl uppercase tracking-tight mb-2">📒 Diccionario de Técnicas y Recursos</h3>
        <p className="text-base font-semibold leading-relaxed max-w-2xl text-neutral-800">
          A continuación se presenta un repositorio práctico con todas las técnicas clave necesarias. 
        </p>
      ...
      <div className="flex border-b-4 border-black overflow-x-auto hide-scrollbar">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id as 'generales' | 'relajacion' | 'concentracion')}
      ...
      <div className="p-4 sm:p-6 bg-white space-y-6 max-h-[600px] overflow-y-auto">
        {getData().map((item, index) => (
          <div key={index} className="p-5 border-4 border-black bg-neutral-50 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col lg:flex-row justify-between gap-6">
            <div className="flex-1">
              <h4 className="text-xl font-black uppercase text-black mb-2">{item.title}</h4>
              <p className="text-sm font-bold text-gray-700 mb-4">{item.desc}</p>
              
              <div className="bg-white border-2 border-black p-4 mb-3">
                <h5 className="font-mono text-xs font-black uppercase mb-2 text-blue-700">[ CÓMO SE REALIZA ]</h5>
                <ol className="list-decimal pl-5 space-y-1">
                  {item.steps.map((step, idx) => (
                    <li key={idx} className="text-[13px] font-bold">{step}</li>
                  ))}
                </ol>
              </div>
              ...
              {item.example && (
                <div className="bg-yellow-100 border-2 border-black p-3 font-mono text-xs font-bold text-black flex gap-2">
                  <span className="shrink-0 uppercase text-yellow-700">Ejemplo:</span>
            ...
            <div className="shrink-0 flex items-center justify-center lg:items-start lg:border-l-4 lg:border-black lg:pl-6">
              ...
              >
                ...
                <span className="text-center text-sm lg:text-base">INICIAR<br className="hidden xl:block" /> SESIÓN</span>
              </button>
            </div>
          </div>
        ))}
